...
public class Main {
	...
	private static void printListings(final Edits edits, final String transactionId)
			throws IOException {
	...
	private static void setListingDescription(final Edits edits, final String transactionId, String language, String description)
			throws IOException {
		Listings listings = edits.listings();
        ListingsListResponse listingsListResponse = listings.list(PACKAGE, transactionId).execute();
        ...
        if (listing == null) {
        	throw new IOException("Could not find listing for language " + language);
        }
